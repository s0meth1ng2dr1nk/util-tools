...
build_ssh_format() {
  ...
  echo "${_ESCAPE}${NEWLINE}${_TAB}sshpass -p ${DOLLER_}PASS${N_}} ssh ${DOLLER_}USER${N_}}@${DOLLER_}IP${N_}} -p ${DOLLER_}PORT${N_}} ${DOLLER_}OPT${N_}} ${_CW} ${_PROXY_COMMAND}@{1}${NEWLINE}${_TAB/${SPACE}/}${_ESCAPE}"
}
build_config_format() {
  local N_=$1
